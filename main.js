    ...
    if (channel.type == "text") {
      channel.send(
        "Welcome to China " +
          ...
          "." +
          " Your social credit score has been set to 50. Use !help to get started and enjoy your stay."
      );
      return;
    }
  });
...
function processCommand(receivedMessage) {
  ...
  if (primaryCommand == "help") {
    if (
      users.isElton(receivedMessage.author.id) ||
      users.isRajah(receivedMessage.author.id) ||
      users.isAbdu(receivedMessage.author.id)
    ) {
      receivedMessage.channel.send("Fuck u figure it out");
    } else {
      receivedMessage.channel.send(
        "Use !praise @name 1-5 to praise your friends for being model citizens.\nUse !report @name 1-5 to report your enemies for crimes against the country.\nUse !score @name to see someone's score."
      );
    }
...
function praiseCommand(arguments, receivedMessage) {
  if (validateArguments(arguments, receivedMessage)) {
      ...
      (err, result) => {
        ...
        if (result.rows[0] != undefined) {
          ...
          if (user == receivedMessage.member.user) {
            receivedMessage.channel.send(
              "In China we are proud of our country, not in ourselves."
            );
          ...
          receivedMessage.channel.send(
            displayName +
              " has not been to China recently but we invite them to join our glorious country."
          );
        }
      }
    );
    ...
    receivedMessage.channel.send(
      "Try using !help. The chinese government is happy to assist."
    );
  }
}
...
function evaluateScore(score, receivedMessage, user) {
  if (score <= 0) {
    receivedMessage.channel.send(
      user +
        "'s social credit score is " +
        score.toString() +
        "." +
        " For their crimes against the Republic of China they will be subject to capital punishment."
    );
  } else if (score <= 10) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " They and their family will be imprisoned and subject to labor for 3 generations."
    );
  } else if (score <= 20) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        "They will be sent to a re-eduation camp in XinJiang"
    );
  } else if (score <= 30) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " They have been put on a list and will be closely monitored"
    );
  } else if (score <= 40) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " They are no longer allowed to ride on public transport."
    );
  } else if (score <= 50) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " Their family will only be deducted one bag of rice."
    );
  } else if (score <= 60) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " The supreme leader has recognized their contribution to the motherland. Their family will be sent an extra bag of rice."
    );
  } else if (score <= 70) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " The supreme leader has found them an opening at T1 Esports as a reward for their excellent behaviour."
    );
  } else if (score <= 80) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " As a model citizen, the supreme leader has gifted them a brand new Tesla."
    );
  } else if (score <= 90) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " For their astounding contributions, the supreme leader has sent them a mail-order bride to procreate with."
    );
  } else if (score <= 100) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " Their upstanding loyalty to the Republic of China has been rewarded with a luxurious estate in Vancouver."
    );
  } else if (score > 100) {
    receivedMessage.channel.send(
      user +
        ...
        score.toString() +
        "." +
        " The Emperor invites you to tea."
    );
  }
}
